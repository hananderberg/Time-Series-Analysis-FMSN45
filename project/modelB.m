...
%y_val_last_month = y_validation(end-30:end);
...
nav_pred = filter([1 -1],1,y_test);

ehat_naive = y_test - nav_pred;
...
f2 = figure;
shiftK_y = round( mean( grpdelay(Fhh_rain, 1) ) );
plot([y_test(1:end-shiftK_y) nav_pred(shiftK_y+1:end)] )
hold on
figProp = get(f2);
line( [pstart pstart], figProp.CurrentAxes.YLim, 'Color', 'red', 'LineStyle', ':' )
xlim([0 N])
