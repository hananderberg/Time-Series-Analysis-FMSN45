...
for t=4:N-k                                     % We use t-3, so start at t=4. As we form a k-step prediction, end the loop at N-k.
    ...
    for k0=2:k
        Ck = [ -yk -x1(t+k0-2) -x1(t+k0-3) h_et(t+k0-1) ]; % C_{t+k|t}
        yk = Ck*A^k*xt(:,t);                    % \hat{y}_{t+k|t} = C_{t+k|t} A^k x_{t|t}
    end
    yhatk_rain(t+k) = yk;                            % Note that this should be stored at t+k.
end
...
for t=4:N-k                                     % We use t-3, so start at t=4. As we form a k-step prediction, end the loop at N-k.
    ...
    % of the earlier predicted values.
    for k0=2:k
        Ck = [ -yk -x2(t+k0-2) h_et(t+k0-1) ]; % C_{t+k|t}
        yk = Ck*A^k*xt(:,t);                    % \hat{y}_{t+k|t} = C_{t+k|t} A^k x_{t|t}
    end
    yhatk_temp(t+k) = yk;                            % Note that this should be stored at t+k.
end
